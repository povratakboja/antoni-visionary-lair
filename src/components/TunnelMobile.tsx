...
export function TunnelMobile({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    // Mobile optimization: disable antialiasing for better performance
    ...
    // Mobile optimization: Reduce star count from 1000 to 300
    ...
    // Mobile optimization: Reduce plane count from 30 to 15
    ...
    for (let i = 0; i < PLANE_COUNT; i++) {
      ...
      // Mobile optimization: Remove random tilt, glow, and frame for performance
      ...
      // Mobile optimization: Simple plane, no group, no extra layers
    ...
    function drawPerspectiveGrid(offset: number, speed: number) {
      ...
      // Mobile optimization: Reduce grid complexity for better performance
      const GRID_DIVISIONS = 8; // Reduced from 12
      const DEPTH_LAYERS = 10; // Reduced from 15
    ...
    function animate() {
      ...
      imagePlanes.forEach((plane) => {
        ...
        // Mobile optimization: Simple opacity, no layers
        ...
        if (plane.position.z > camera.position.z + 15 && !inExitPhase) {
          ...
          // Change to new texture (mobile: no random tilt)
    ...
    return () => {
      ...
      // Dispose image planes (mobile: simple planes, no groups)
      imagePlanes.forEach(plane => {
        if (plane) {
