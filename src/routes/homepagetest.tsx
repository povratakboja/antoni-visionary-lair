...
export const Route = createFileRoute("/homepagetest")({
  head: () => ({
    meta: [
      { title: "ˈvɪʒəˌnɛɹi — Antoni Bonačić Vičić (Test)" },
...
function HomepageTest() {
  ...
  const handleHoverFlash = useCallback(() => {
    // Only trigger flash on first hover
    if (!hasHoveredOnce) {
      triggerFlash(0.18, 1);
      setHasHoveredOnce(true);
    }
  }, [triggerFlash, hasHoveredOnce]);

  const handleClick = useCallback(
    (n: number) => {
      // Reset hover flag on click so next hover will flicker again
